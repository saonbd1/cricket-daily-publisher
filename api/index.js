...
function persistedVerificationFixture(fixture, saved) {
  let sourceEvidence = fixture.sourceEvidence;
  if (saved.sourceEvidence) {
    try {
      const parsed = JSON.parse(saved.sourceEvidence);
      if (Array.isArray(parsed)) sourceEvidence = parsed.filter((value) => typeof value === "string");
    } catch {
...
async function runPublisher(trigger) {
  ...
  try {
    ...
    return { runId, status: finalStatus, fixturesFetched, postsCreated, postsUpdated, verification: { ...reconciled, verified: effectiveVerified, candidates: effectiveCandidates, conflicts: effectiveConflicts } };
  } catch (error) {
