...
import { Activity, AlertTriangle, ExternalLink, Play, Radio, RefreshCw, ShieldCheck, Trophy } from "lucide-react";
...
export default function Home() {
  ...
  const verificationQueue = trpc.publisher.verificationQueue.useQuery();
      ...
      <div className="min-h-screen bg-[#07111f] text-slate-100 -m-4 p-4 md:p-8">
        <div className="mx-auto max-w-7xl space-y-8">
          ...
          <section className="grid gap-4 md:grid-cols-4">
            ...
            <Card className="border-amber-300/20 bg-[#0c1a2e] text-slate-100"><CardHeader className="pb-3"><CardTitle className="flex items-center justify-between text-sm font-medium text-slate-300">Review queue <AlertTriangle className="h-4 w-4 text-amber-300" /></CardTitle></CardHeader><CardContent><div className="text-3xl font-black">{verificationQueue.data?.length ?? 0}</div><p className="mt-2 text-xs text-slate-400">Candidates and conflicts held out of Blogger.</p></CardContent></Card>
          </section>
          ...
          <section><div className="mb-3 flex items-center justify-between"><div><h2 className="text-lg font-bold">Verification review queue</h2><p className="mt-1 text-xs text-slate-400">Only fixtures confirmed by both structured sources are published automatically.</p></div><Badge variant="outline" className="border-amber-300/30 text-amber-200">{verificationQueue.data?.length ?? 0} held</Badge></div><Card className="border-amber-300/15 bg-[#0c1a2e] text-slate-100"><CardContent className="p-0"><div className="divide-y divide-slate-700/50">{(verificationQueue.data ?? []).slice(0, 12).map(({ fixture, tournament, sourceEvidence }) => <div key={fixture.id} className="flex flex-wrap items-center justify-between gap-4 px-5 py-4"><div><div className="font-semibold">{fixture.teamOne} <span className="text-slate-500">vs</span> {fixture.teamTwo}</div><div className="mt-1 text-xs text-slate-400">{tournament.name} · {fixture.localDateGmt6} · {fixture.localTimeGmt6} GMT+6</div><div className="mt-1 text-xs text-slate-500">Sources: {sourceEvidence.length ? sourceEvidence.join(", ") : "not recorded"}</div></div><div className="flex items-center gap-3"><Badge variant="outline" className={fixture.verificationStatus === "conflict" ? "border-red-400/30 text-red-300" : "border-amber-400/30 text-amber-200"}>{fixture.verificationStatus}</Badge><span className="text-xs text-slate-500">Not published</span></div></div>)}{(verificationQueue.data ?? []).length === 0 && <div className="px-5 py-8 text-center text-slate-400">No candidate or conflict fixtures are waiting.</div>}</div></CardContent></Card></section>

          <section><h2 className="mb-3 text-lg font-bold">Publisher run history</h2><Card className="border-cyan-300/15 bg-[#0c1a2e] text-slate-100"><CardContent className="p-0"><div className="divide-y divide-slate-700/50">{(runs.data ?? []).slice(0, 8).map(run => <div key={run.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-3 text-sm"><div><span className="font-medium capitalize">{run.trigger}</span><span className="ml-3 text-xs text-slate-400">{new Date(run.startedAt).toLocaleString()}</span></div><div className="flex items-center gap-3 text-xs text-slate-400"><span>{run.fixturesFetched} fixtures</span><span>{run.postsCreated} created</span><span title={run.errorMessage ?? undefined}>{run.status === "partial" ? "Review needed" : "Verified"}</span><span>{run.postsUpdated} updated</span><span>API {run.apiStatusCode ?? "—"} · Blogger {run.bloggerStatusCode ?? "—"}</span>{run.postUrls && <span className="max-w-[22rem] truncate" title={run.postUrls}>{run.postUrls}</span>}<Badge variant="outline" className={statusTone(run.status)}>{run.status}</Badge></div></div>)}{(runs.data ?? []).length === 0 && <div className="px-5 py-8 text-center text-slate-400">No publisher runs yet.</div>}</div></CardContent></Card></section>
        </div>
      </div>
    </DashboardLayout>
  );
